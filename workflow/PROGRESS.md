...
## Iteration: 2026-07-08 (Part 3)
- **Target Range**: 0x3E29-0x3FFF (German PDF pages 258-265; the rest of the ROM to its end, starting on the untouched code just after the error table)
- **Prompt**: "We just need to translate pages 258-265 and annotate the rest of the rom. A few things that are totally called out in the markdowns, sometimes the listing skips memory ranges so watch for that, typically this is dead code not part of the rom or its null values. ... this would be starting on the untouched code in export.lst on line 9907 and starting at 0x3E27."
- **Summary**: Translated and annotated the remainder of the ROM's actual code, covering 18 routines: buffer-blanking helper (`BUF_BLANK_IF_EMPTY`), initialization color setup (`INIT_COLOR_YELLOW`), the green-background RDLINE screen-to-buffer transfer (`RDLINE_READ_GREEN_BG` / `RDLINE_READ_ENTRY`), character inversion helpers (`CHAR_INVERT`, `CHAR_OUT_INVERT_ADJUST`, `CHAR_OUT_BG_ADJUST`), screen-clear character helpers (`CLEAR_CHAR_GET`, `CLRLINE_CHAR_GET`), blank-character testing/insertion (`CHAR_IS_BLANK_TEST`, `INSERT_BLANK_CHAR`), several cassette-related background-aware print helpers (`TAPE_MSG_PRINT_BG`, `TAPE_ID_PRINT_BG`, `TAPE_NAME_PRINT_BG`, `TAPE_READ_BYTE_CHECKED`), the COPY-statement character helper (`COPY_CHAR_BG`), the background-color-change screen-conversion routine (`BG_COLOR_CHANGE_CONVERT`), and the CTRL-key-during-init check (`CTRL_KEY_CHECK_INIT`). Added 17 new symbols and 194 inline comments + 18 block headers across 221 lines (0x3E29-0x3FB6). Confirmed the real end of ROM code is 0x3FB6; marked 0x3FB7-0x3FEF (0x00 fill) and 0x3FF0-0x3FFF (0xFF fill, standard unprogrammed-EPROM signature) as `DIRECT_BYTE` "unused ROM space" rather than leaving them mis-labeled as executed `nop`/`rst 0x38` code. No address gaps were found in this range (double-checked programmatically) - the whole 0x3E29-0x3FB6 span is contiguous real code.
- **Toolchain bug found and fixed (rebuilt from source, `annotate.c`)**: multi-line `block` (and `comment`) annotation text was silently truncated whenever a continuation line contained an `=` character (e.g. "Set default color = yellow"). Root cause: the `.ann` file parser ran an unconditional `strchr(line,'=')` on every line including indented continuation lines, so a continuation line with `=` got misparsed as a new (invalid, silently-dropped) key/value pair, which also reset the "currently appending" pointer, truncating everything after it. Fixed by only attempting key/value parsing on non-indented lines (continuation lines are always written indented by this tool), verified with a standalone repro before and after the fix, then re-verified every block set this session against its expected line count.
- **Statistics**: Symbols: 448, Segments: 516.
- **Verification**: `rom.bin`/`export.bin` MD5 unchanged (`42c8f9e6c2133ae0e953b89ccbbdb7e2`). Programmatic check confirmed 221 addressable lines in range, 194 with new comments and exactly the 27 deliberately-uncommented continuation/shared-target lines expected from the translation plan. All 18 block annotations re-verified to have their expected line count after the parser fix.
- **Remaining work**: everything past 0x3FFF (i.e. the rest of the PDF book) is the staged memory-layout/appendix section, not ROM code - the user plans to do that pass by hand.
